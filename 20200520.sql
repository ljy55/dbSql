...
FROM emp;
--위 쿼리와 아래 쿼리는 실행결과 동일하나 위 쿼리가 더 명확해서 추천
...
20200501~20200630 : 61개행
dt 컬럼에는 데이터가 5/8~6/7에 해당하는 데이트 타입 자료가 저장되어 있는데
5/1~5/31에 해당하는 날짜(년월일)을 중복없이 조회하고 싶다
...
우리가 원하는 답의 최대 행의 결과 : 24행 ==> 31개의 행이 최대답(5월 전체로 봤을 때)
