...
SQL 연산 : JAVA와 다르게 개입 X, 일반적 사칙연산
int b = 2; =대입 연산자, == 비교;
...
WHERE 절에 기술한 조건이 참(TRUE)일 때 결과를 조회 --중요
...
'1234', 1234 --이 둘은 다름(앞 : 문자,  뒤: 숫자)

emp테이블 컬럼과 데이터 타입을 확인;
DESC emp;
...
emp 테이블에서 직원이 속한 부서번호가 30번보다 크거나 같은 부서에 속한 직원을 조회;
SELECT *
FROM emp
WHERE deptno >= 30;
