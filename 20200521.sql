...
연말정산 : 소득의 25%를 신용카드 + 현금영수증 + 체크카드로 소비를
          ...
          신용카드는 공제 : 15%, 현금영수증은 30%, 체크카드 30%를 공제하는 
...
날짜를 입력받아 ==> 그 회사의 영업일기준으로 5일뒤의 날짜를 리턴하는 함수
회사만의 특이한 로직이 필요한 경우 함수로 만들 수 있다

PROCEDURE : 이름이 없는 PL/SQL 블록, 리턴값이 없다
...
PL/SQL에서는 SELECT 쿼리를 실행했을 때 데이터가 한건 안나올 경우 NO_DATA_FOUND 예외를 던진다
...
DECLARE
    ...
    --새롭게 만든 타입으로 변수를 선언(java로 생각하면 class를 만들고 인스턴스 생성)
    v_dept_rec dept_rec;
BEGIN
...
dept테이블의 행 정보를 저장할 수 있는 테이블 TYPE
    ...
    intArrat["첫번쨰"] = 50;
    System.out.ptintln(intArray("첫번째));
    
    PL/SQL에서는 두가지 타입을 지원 : 정수(BINARY_INREGER), 문자열(VARCHAR(2))
